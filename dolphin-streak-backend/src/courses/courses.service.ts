...
export class CoursesService {
  constructor(
    ...
    private courseSessionModel: Model<CourseSessionDocument>,
  ...
  async addSession(data: any): Promise<CourseSession> {
    return await this.courseSessionModel.create(data);
  }

  async getOneSession(id: string): Promise<CourseSession> {
    return this.courseSessionModel.findById(id);
  }
  ...
  async assessAnswer(
    ...
    if (qtype == 'MULTIPLE_CHOICE') {
      ...
      if (
        answer.toLowerCase() == question.answerOptions[answerIdx].toLowerCase()
      ) {
        return { suggestion: null, isCorrect: true };
      }
    } else if (qtype == 'FILL_IN') {
      let correctOne: string;
      ...
      try {
        ...
        return {
          suggestion: 'Your pronunciation is wrong. Try again!',
          isCorrect: false,
        };
      } catch (error) {
        console.error(
